...
def load_and_preprocess_data(file_path):
    try:
        ...
        df['Time_Spent_Hours'] = (df['Logout Time'] - df['Login Time']).dt.total_seconds() / 60  # Convert to minutes
        ...
        median_time = df['Time_Spent_Hours'].median()
        median_attempts = df['Count of Survey Attempts'].median()
        df['Likely_Survey'] = ((df['Time_Spent_Hours'] > median_time) | (df['Count of Survey Attempts'] > median_attempts)).astype(int)

        features = ['Speciality_Encoded', 'Region_Encoded', 'Login_Hour', 'Logout_Hour', 'Time_Spent_Hours', 'Count of Survey Attempts']
